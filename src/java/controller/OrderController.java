...
public class OrderController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (service.equals("listCustomerOrders")) {
                String customerId = request.getParameter("customerId");

                if (customerId == null || customerId.trim().isEmpty()) {
                    response.sendRedirect("OrderURL?service=listAllOrder&message=CustomerId is required");
                    return;
                }

                String sql = "SELECT * FROM [Order] WHERE CustomerId = ?";
                Vector<Order> vector = dao.getOrderByCustomerId(Integer.parseInt(customerId));


                RequestDispatcher dispath = request.getRequestDispatcher("/orders.jsp");
                request.setAttribute("data", vector);
                request.setAttribute("title", "Orders of Customer ID: " + customerId);
                dispath.forward(request, response);
            }
    ...
    if ("listCustomerOrders".equals(service)) {
        ...
        List<Order> list = dao.getOrderByCustomerId(customerId);
